...
<script src="https://cdn.rawgit.com/rainabba/jquery-table2excel/1.1.2/dist/jquery.table2excel.min.js"></script>
<style>
.flota{
	overflow: hidden;
    position: sticky;
	...
	<table class="table table-sm table-borderless" id="resultsTable">
		<thead>
			...
			<tr title="{{$d->sucursal}} - {{$d->bodega}} - {{number_format($d->min,0)}} - {{number_format($d->max,0)}}">
				<th style="color:black; background:#EEBC717D;">{{$d->sucursal}} - {{$d->bodega}}</th>
			@endif
			...
			<tr title="{{$d->sucursal}} - {{$d->bodega}} - {{number_format($d->min,0)}} - {{number_format($d->max,0)}}">
				<th style="color:black; background:#ED696975;">{{$d->sucursal}} - {{$d->bodega}}</th>
			@endif
			@php $currentSuc = $d->cod_unidad @endphp
			@php $currentBod = $d->cod_bodega @endphp
			@php $currentNom = $d->bodega @endphp
			...
			@if($d->Cod_Cliente == '6815' || $d->Cod_Cliente == '6816')
			@if($d->existencia < $d->min)
				<td style="background-color:#B6AAE180; color:red;">{{number_format($d->existencia,0)}}</td>
			@elseif($d->existencia >= $d->min && $d->existencia <= $d->max)
				<td  style="background-color:#B6AAE180; color:orange;">{{number_format($d->existencia,0)}}</td>
			@else
				<td  style="background-color:#B6AAE180; color:black;">{{number_format($d->existencia,0)}}</td>
			@endif
			@endif
			@php $currentSuc = $d->cod_unidad @endphp
			@php $currentBod = $d->cod_bodega @endphp
			...
			@if($d->Cod_Cliente == '6814')
			@if($d->existencia < $d->min)
				<td style="background-color:#EE1A1AB8; color:white;">{{number_format($d->existencia,0)}}</td>
			@elseif($d->existencia >= $d->min && $d->existencia <= $d->max)
				<td  style="background-color:#66BC11AB; color: black;">{{number_format($d->existencia,0)}}</td>
			@else
			...
			@foreach($datos as $d)
			@if($d->bodega != $currentNom)
			@if($d->cod_unidad > 27 && $d->cod_unidad < 32)
			<tr title="{{$d->sucursal}} - {{$d->bodega}} - {{number_format($d->min,0)}} - {{number_format($d->max,0)}}">
				<th style="color:white;background:#343434;">{{$d->sucursal}} - {{$d->bodega}}</th>
			@endif
			@php $currentSuc = $d->cod_unidad @endphp
