<template>
    <div class="container-fluid container-task">
        ...
        <input class="form-control" type="text" v-model="searchQuery" placeholder="Buscar producto">
            ...
            <table class="table table-sm table-borderless" v-if="arrayTasks.length" >
                ...
                <tbody>
                    <tr v-for="item in filteredResources" :key="item.id">
                        ...
                        <td v-text="item.nombre_corto" style="text-transform: uppercase;"></td>
                        <template v-if="item.id_superviso == 1">
                        <td v-text="item.nombre_fiscal" style="background-color:#F78D3D8F"></td>
                        </template>
                        <template v-else>
                        ...
                        <td><a href="#" class="badge badge-danger" @click="loadFieldsUpdate(item)" data-toggle="modal" data-target="#actualizar" aria-pressed="false"> Editar</a></td>
                        <td><span class="badge badge-dark" @click="deleteTask(item)">Eliminar</span></td>
                    </tr>
                </tbody> 
                <tfoot>
                    <tr>
            ...
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                            ...
                            <table class="table table-sm table-borderless">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td style="background-color:#3FB4ED75" v-text="max - sucursal"></td>
                                        <td v-text="min"></td> 
                                        <td v-text="reo"></td> 
                                        <td v-text="max"></td>
                                    </tr>
                        ...
                        <form v-on:submit.prevent="updateTasks()" id="actualizar_Producto">
                            <div class="form-row">
                                <div class="form-group col-md-3">
                                    <label>Sugerido sucursal</label>
                                    <input v-model="cantidadSu" type="number" class="form-control" id="sugerido" placeholder="0" disabled>
                                </div>
                                <div class="form-group col-md-3">
                                    <label>Cantidad a enviar</label>
                                    <input v-model="cantidad" type="number" class="form-control" id="cantidad" placeholder="0" required>
                                </div>
                                <div class="form-group col-md-3"><!-- Formulario para la creación o modificación de nuestras tareas-->
                                    <label>Bultos</label>
                                    <input v-model="bultos" type="number" step="1" class="form-control" placeholder="0" id="bultos">
                                </div>
                                <div class="form-group col-md-3">
                                    <label>Marcar como importante</label>
                                    <input type="checkbox" class="form-control" id="Si" value="1" v-model="importante">
                                </div>
                                <div class="form-group col-md-12">
                                    <!-- Botón que modifica la tarea que anteriormente hemos seleccionado, solo se muestra si la variable update es diferente a 0-->
                                    <button  @click="updateTasks()" class="btn btn-dark btn-sm btn-block" data-dismiss="modal">Guardar</button>
                                </div>
                            </div>
                        </form>
                    </div>
        ...
        methods:{
            ...
            loadFieldsUpdate(data){ //Esta función rellena los campos y la variable update, con la tarea que queremos modificar
                ...
                axios.get(url).then(function (response) {
                    ...
                    me.importante = response.data.id_superviso;
                })
                .catch(error => {
                    console.log(error)
                    this.errored = true
